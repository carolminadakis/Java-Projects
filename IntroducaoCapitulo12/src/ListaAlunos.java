	...
	void ordenar () {
		for (int i = 1; i < tamanhoLista; i++) {
			...
			while (indicePosicaoBase > 0) {
				int indicePosicaoInicial = indicePosicaoBase - 1;
				Aluno alunoPosicaoInicial = lista [indicePosicaoInicial];
				
				Boolean alunoOrdemAlfabetica = alunoPosicaoInicial == null || alunoPosicaoInicial.vemDepois (alunoPosicaoBase);
				
				if (alunoOrdemAlfabetica) {
					lista [indicePosicaoBase] = lista [indicePosicaoInicial];
